...
export function DashboardPage() {
  ...
  useEffect(() => {
    api<Health>("/api/v1/health/live", {}, token).then(setHealth).catch(() => setHealth(null));
    api<Readiness>("/api/v1/health/ready", {}, token).then(setReadiness).catch(() => setReadiness(null));
  ...
  return (
    <div>
      ...
      <Surface title="Системное состояние"><div className="row"><StatusPill tone={health?.status === "ok" ? "ok" : "danger"}>API: {health?.status || "недоступен"}</StatusPill><StatusPill tone={readiness?.status === "ok" ? "ok" : "danger"}>PostgreSQL / Redis: {readiness?.status || "недоступны"}</StatusPill>{health && <span className="muted">v{health.version} · {health.env}</span>}</div></Surface>
