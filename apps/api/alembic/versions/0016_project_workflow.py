...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column("domain_check_meta", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
    ...
    op.create_table(
        ...
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False),
        sa.Column("supersedes_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("project_fact_revisions.id", ondelete="SET NULL"), nullable=True),
    ...
    op.create_index("ix_project_fact_revisions_project_id", "project_fact_revisions", ["project_id"])
    op.create_index("ix_project_fact_revisions_state", "project_fact_revisions", ["state"])
    op.create_foreign_key("fk_projects_current_fact_revision", "projects", "project_fact_revisions", ["current_fact_revision_id"], ["id"], ondelete="SET NULL")
    ...
    op.create_table(
        ...
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("project_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("projects.id", ondelete="CASCADE"), nullable=False),
        sa.Column("geo_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("geo_places.id", ondelete="RESTRICT"), nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False),
        sa.Column("role", sa.String(32), nullable=False, server_default="service_area"),
        sa.Column("position", sa.Integer(), nullable=False, server_default="0"),
        sa.Column("morph_overrides", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()")),
    ...
    op.create_table(
        ...
        sa.Column("project_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("projects.id", ondelete="CASCADE"), nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False),
        sa.Column("supersedes_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("page_plans.id", ondelete="SET NULL"), nullable=True),
        ...
        sa.Column("block_selection", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("fact_revision_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("project_fact_revisions.id", ondelete="SET NULL"), nullable=True),
        sa.Column("keyword_snapshot", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("geo_snapshot", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.text("now()")),
        sa.UniqueConstraint("project_id", "slug", "version", name="uq_page_plan_project_slug_version"),
    )
    ...
    op.create_table(
        "page_drafts",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("page_plan_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("page_plans.id", ondelete="CASCADE"), nullable=False),
        sa.Column("project_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("projects.id", ondelete="CASCADE"), nullable=False),
        sa.Column("tenant_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.Column("page_manifest", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("generator_meta", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
        sa.Column("content_hash", sa.String(64), nullable=True),
        sa.Column("qa_runs", postgresql.JSONB(), nullable=False, server_default=sa.text("'[]'::jsonb")),
        sa.Column("last_qa_verdict", sa.String(16), nullable=True),
        sa.Column("qa_override", postgresql.JSONB(), nullable=False, server_default=sa.text("'{}'::jsonb")),
    ...
    op.create_foreign_key("fk_sites_project_id", "sites", "projects", ["project_id"], ["id"], ondelete="SET NULL")
    op.create_unique_constraint("uq_sites_project_id", "sites", ["project_id"])
    op.create_index("ix_sites_project_id", "sites", ["project_id"])

    op.add_column("site_pages", sa.Column("project_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column("site_pages", sa.Column("page_plan_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column("site_pages", sa.Column("page_draft_id", postgresql.UUID(as_uuid=True), nullable=True))
    op.create_foreign_key("fk_site_pages_project_id", "site_pages", "projects", ["project_id"], ["id"], ondelete="SET NULL")
    op.create_foreign_key("fk_site_pages_page_plan_id", "site_pages", "page_plans", ["page_plan_id"], ["id"], ondelete="SET NULL")
    op.create_foreign_key("fk_site_pages_page_draft_id", "site_pages", "page_drafts", ["page_draft_id"], ["id"], ondelete="SET NULL")
    for column in ("project_id", "page_plan_id", "page_draft_id"):
        op.create_index(f"ix_site_pages_{column}", "site_pages", [column])

    op.add_column("site_builds", sa.Column("project_id", postgresql.UUID(as_uuid=True), nullable=True))
    ...
    op.add_column("site_builds", sa.Column("requested_by", postgresql.UUID(as_uuid=True), nullable=True))
    op.add_column("site_builds", sa.Column("activated_at", sa.DateTime(timezone=True), nullable=True))
    op.create_foreign_key("fk_site_builds_project_id", "site_builds", "projects", ["project_id"], ["id"], ondelete="SET NULL")
    ...
    op.create_table(
        ...
        sa.Column("page_plan_id", postgresql.UUID(as_uuid=True), sa.ForeignKey("page_plans.id", ondelete="SET NULL"), nullable=True),
...
def downgrade() -> None:
    ...
    for column in ("activated_at", "requested_by", "page_plan_ids", "manifest_snapshot", "project_id"):
